    ...
    <div style="display: flex;">
      <div style="flex: 50%;">
        <div style="border: 1px dashed blue;">
          ...
          <ul>
            <li v-for="(item, index) of state.report.incomes" :key="`incomes-${index}`">
              {{ JSON.stringify(item) }}
            </li>
          </ul>
        </div>
        <div style="border: 1px dashed blue;">
          ...
          <ul>
            <li v-for="(item, index) of state.report.expenses" :key="`expenses-${index}`">
              {{ JSON.stringify(item) }}
            </li>
          </ul>
        </div>
        <div style="border: 1px dashed blue;">
          ...
          <ul>
            <li v-for="(item, index) of state.report.assets" :key="`assets-${index}`">
              {{ JSON.stringify(item) }}
            </li>
          </ul>
        </div>
        <div style="border: 1px dashed blue;">
          ...
          <ul>
            <li v-for="(item, index) of state.report.liabilities" :key="`liabilities-${index}`">
              {{ JSON.stringify(item) }}
            </li>
          </ul>
        </div>
      </div>
      <div style="flex: 50%; border: 1px dashed green; display: flex; flex-direction: column;">
        <template v-if="currentFlow">
          <h4>{{ currentFlow.flowType }}</h4>
          <div v-if="currentFlow.flowType === FlowType.BIRTH_CHILD" style="display: flex; flex-direction: column; flex-grow: 1;">
          ...
          <div v-if="currentFlow.flowType === FlowType.RECEIVING_SALARY" style="display: flex; flex-direction: column; flex-grow: 1;">
            <div style="flex-grow: 1;">
              Получение зарплаты
    ...
    addFlow(flow) {
      ...
      switch (flowType) {
        case FlowType.RECEIVING_SALARY:
          {
            ...
            let assetIndex = assets.findIndex(x => x.flowType === FlowType.CASH)
            if(assetIndex > -1){
              let totalExternal = assets[assetIndex].value + total
              this.$store.commit("setAsset", { index: assetIndex, flow: { flowType: FlowType.CASH, value: totalExternal }})
            }else{
              this.$store.commit("addAsset", { flowType: FlowType.CASH, value: total })
            }
          }
          break;
        case FlowType.JOB:
          {
            ...
            let incomeIndex = incomes.findIndex(x => x.flowType === FlowType.JOB)
            if(incomeIndex > -1){
        ...
        case FlowType.GENERAL_EXPENSES:
          {
            ...
            let expenseIndex = expenses.findIndex(x => x.flowType === FlowType.GENERAL_EXPENSES);
            let { value } = flow
            if(expenseIndex > -1){
              let total = value + expenses[expenseIndex].value
              this.$store.commit("setExpense", { index: expenseIndex, flow: { flowType, value: total } })
            }else{
        ...
        case FlowType.BIRTH_CHILD:
          {
            let { expenses } = this.state.report;
            let expenseIndex = expenses.findIndex(x => x.flowType === FlowType.BIRTH_CHILD);
            ...
            if(expenseIndex > -1){
              ...
              this.$store.commit("setExpense", { index: expenseIndex, flow: { flowType, value: totalValue, count: totalCount } })
            }else{
